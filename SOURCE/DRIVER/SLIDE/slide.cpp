...
static const pd4Nanotec::_OD_InitVector slide_config_vector[]={
...
static const pd4Nanotec::_OD_InitVector slide_runtime_vector[]={
...
static const pd4Nanotec::_OD_InitVector slide_zero_vector[]={
...
slideModule::slideModule(void):pd4Nanotec(CONFIG->getParam<uint>(SLIDE_CANID, 0))
{
    setConfigVector((_OD_InitVector*) slide_config_vector);// This is the register vector that will be stored if necessary
    setRuntimeVector((_OD_InitVector*) slide_runtime_vector);// Initialize the device with the Main OD vector
    setZerosettingVector((_OD_InitVector*) slide_zero_vector);// Initialize the device with the zero setting vector
...
void slideModule::initializationCompleted(bool result){
    INTERFACE->EVENT_SlideInitCompleted(result);
}
