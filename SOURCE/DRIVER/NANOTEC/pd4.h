...
public:
    ...
    _inline bool isDeviceInitialized(void){return (deviceInitialized);}
    bool activateInitialization(void);
...
protected:
    ...
    void readSDO(ushort index, uchar sub, uchar type, uint repeat=0);
    void readSDO(canOpenDictionary od);
    void resetNode(void);


    virtual void initializationCompleted(bool result){return;}
...
private slots:
    ...
    void rxAsyncFromCan(ushort canId, QByteArray data);//!< Receive Async Can data frame from the canDriver

    ushort workflowUploadNanoj(void); //!< Workflow of Upload Nanoj-program


    ushort workflowInitializeHandler(void);
