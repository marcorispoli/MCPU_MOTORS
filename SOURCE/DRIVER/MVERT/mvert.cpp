...
static const pd4Nanotec::_OD_InitVector mvert_config_vector[]={
...
static const pd4Nanotec::_OD_InitVector mvert_runtime_vector[]={
...
static const pd4Nanotec::_OD_InitVector mvert_zero_vector[]={
...
mvertModule::mvertModule(void):pd4Nanotec(CONFIG->getParam<uint>(MVERT_CANID, 0))
{
    setConfigVector((_OD_InitVector*) mvert_config_vector);// This is the register vector that will be stored if necessary
    setRuntimeVector((_OD_InitVector*) mvert_runtime_vector);// Initialize the device with the Main OD vector
    setZerosettingVector((_OD_InitVector*) mvert_zero_vector);// Initialize the device with the zero setting vector
...
void mvertModule::initializationCompleted(bool result){
    INTERFACE->EVENT_MvertInitCompleted(result);
}
