...
static const pd4Nanotec::_OD_InitVector carm_config_vector[]={
...
static const pd4Nanotec::_OD_InitVector carm_runtime_vector[]={
...
static const pd4Nanotec::_OD_InitVector carm_zero_vector[]={
...
carmModule::carmModule(void):pd4Nanotec(CONFIG->getParam<uint>(CARM_CANID, 0))
{
    setConfigVector((_OD_InitVector*) carm_config_vector);// This is the register vector that will be stored if necessary
    setRuntimeVector((_OD_InitVector*) carm_runtime_vector);// Initialize the device with the Main OD vector
    setZerosettingVector((_OD_InitVector*) carm_zero_vector);// Initialize the device with the zero setting vector
...
void carmModule::initializationCompleted(bool result){
    INTERFACE->EVENT_CarmInitCompleted(result);
}
