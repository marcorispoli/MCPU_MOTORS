...
static const pd4Nanotec::_OD_InitVector body_config_vector[]={
...
static const pd4Nanotec::_OD_InitVector body_runtime_vector[]={
...
static const pd4Nanotec::_OD_InitVector body_zero_vector[]={
...
bodyModule::bodyModule(void):pd4Nanotec(CONFIG->getParam<uint>(BODY_CANID, 0))
{
    setConfigVector((_OD_InitVector*) body_config_vector);// This is the register vector that will be stored if necessary
    setRuntimeVector((_OD_InitVector*) body_runtime_vector);// Initialize the device with the Main OD vector
    setZerosettingVector((_OD_InitVector*) body_zero_vector);// Initialize the device with the zero setting vector
...
void bodyModule::initializationCompleted(bool result){
    INTERFACE->EVENT_BodyInitCompleted(result);
}
